...
calc = StringVar()
calc= Entry(window, width=31, textvariable=calc, relief=SUNKEN, font="Roboto 20")
calc.grid(row=0, column=0, columnspan=10, padx=11, pady=12)
...
def sciCal(event):
    ...
    calc.delete(0, END)
    if text == "sin":
        calc.insert(0, m.sin(float(val)))
    elif text == "cos":
        calc.insert(0, m.cos(float(val)))
    elif text == "tan":
        calc.insert(0, m.tan(float(val)))
    elif text == "log":
        if (float(val) <= 0.00):
            calc.insert(0, "Not Possible")
        else:
            calc.insert(0, m.log10(float(val)))
    elif text == "ln":
        if (float(val) <= 0.00):
            calc.insert(0, "Not Possible")
        else:
            calc.insert(0, m.log(float(val)))
    elif text == "√":
        calc.insert(0, m.sqrt(float(val)))
    elif text == "!":
        calc.insert(0, m.factorial(int(val)))
    elif text == "rad":
        calc.insert(0, m.radians(float(val)))
    elif text == "deg":
        calc.insert(0, m.degrees(float(val)))
    elif text == "1/x":
        if (val == "0"):
            calc.insert(0, "ꝏ")
        else:
            calc.insert(0, 1 / float(val))
    elif text == "π":
        if val == "":
            ans = str(m.pi)
            calc.insert(0, ans)
        ...
        if val == "":
            calc.insert(0, str(m.e))
        else:
            calc.insert(0, str(float(val) * (m.e)))
